...
const steps = [
  {
    ...
    pill: "bg-pantry-green/10 text-pantry-green",
    shift: "ml-0 mr-auto",
  },
  {
    step: "02",
    ...
    pill: "bg-pantry-amber/10 text-pantry-amber",
    shift: "sm:ml-[14%]",
  },
  {
    step: "03",
    ...
    pill: "bg-pantry-coral/10 text-pantry-coral",
    shift: "sm:ml-[28%]",
  },
];
...
function Step({ step, title, body, color, pill, shift, index }: typeof steps[0] & { index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-6 ${shift}`}
    >
      {/* Big number — fixed width so text always aligns */}
      <span
      ...
      <div className="flex flex-col gap-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className={`${pill} text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0`}>
            Step {step}
          </span>
        </div>
